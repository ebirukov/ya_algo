...
//комнате. Кондиционер достаточно мощный, поэтому при настройке на правильный
//режим работы он за час доводит температуру в комнате до желаемой. Требуется
...
func Conditioner() {
	...
	res := work(tRoom, tCond, mode)
	fmt.Printf("%d", res)
}

func work(room int, cond int, mode string) int {
	switch mode {
	case "freeze":
		if room <= cond {
			return room
